...
Butterfly Backup is a modern backup program written in python; this means the first requirements is python3.8 or higher.
...
Butterfly Backup it is a server to client tool. It must be installed on a server (for example a workstation machine),
...
* **destination**: it is the destination of all backup of all machine under Butterfly Backup system; this is the **catalog**.
* **hostname or ip of the machine under backup**: it is the root of every machine under backup; contains all the backups.
* **timestamp folder**: this is a single backup folder; contains all the data that have been backed up.
* **backup.log**: if enabled logging, it is the log generated by rsync for backup operation.
* **restore.log**: if enabled logging, it is the log generated by rsync for restore operation.
* **general.log**: if enabled logging, it is the general log of the machine; contains all the info, warnings and errors generated during the backup.
* **symlink of last backup**: this is a symbolic link of the last backup made. Isn't supported if you use an MS-DOS file system.
* **backup.list**: if enabled logging, it is the log generated by *list* operation.
* **export.log**: if enabled logging, it is the log generated by *export* operation.
* **.catalog.cfg**: is the log that contains the catalog of all backups. This file is the reference of Butterfly Backup to perform future backups.
...
* **config**: operations involving OpenSSH and its configurations (optional).
