...
Backups are organized according to precise cataloging; this is an example:
...
.. code-block:: bash
   ...
   .
   ├── destination
   │   ├── host or ip of the PC under backup
   │   │   ├── timestamp folder
   │   │   │   ├── backup folders
   │   │   │   ├── backup.log
   │   │   │   └── restore.log
...
* **destination**: is the destination of all backup of all machine under Butterfly Backup system; this is root.
* **host or ip of the PC under backup**: is the root of every machine under backup; contains all the backups.
...
* **archive**: transaction that zip old backup and move to new destination.
...
Two macro-options is available:
...
Once you have created the configuration, should be installed (copied) on the machine or on the machines.
...
This command will try to copy the configuration with the current user. If you want to use a different user (as root), run this:
...
For support Butterfly Backup on Windows os install cygwin.
Download https://cygwin.com/ cygwin and follow this instructions for install necessary packages.
...
Backup modes are two; single and bulk. Let's see how to go about looking at the help:
...
.. code-block:: bash
   ...
   Backup options:
     ...
     --user USER, -u USER  User of which you want to backup
...
* **--mode**, **-m**: Select how rsync perform backup:
    ...
    * **Full** -> Completely backup.
    * **Incremental** -> Incremental backup is based on the latest Full (the same files are linked with hard link).
                         A Full is executed if this mode fails to find one.
    * **Mirror** -> Complete mirror backup. If a file in the source no longer exists, deletes it from the destination.
...
* **--user**, **-u**: User of which you want to backup.
...
.. code-block:: bash
   ...
   Restore options:
     ...
     --backup-id ID, -i ID
                           Backup-id of backup
     --user USER, -u USER  User of which you want to perform restore
     --computer HOSTNAME, -c HOSTNAME
...
* **--catalog**, **-C**: Select the backups folder (root).
* **--backup-id**, **-i**: Select backup id in the catalog.
* **--user**, **-u**: User of remote machine if you want to restore.
